...
(function (factory) {
  ...
  function _int2time(timeInt, settings) {
    ...
    let carryover = 0;
    if (hours > 24) {
      carryover = (hours - (hours % 24)) / 24;
      hours = hours % 24;
    }
    //console.log(carryover, hours, minutes);
    var time = new Date(1970, 0, 2 + carryover, hours, minutes, seconds, 0);
  ...
  function _time2int(timeString, settings) {
    ...
    if (hour <= 12 && ampm) {
      ...
      if (t >= _ONE_DAY + (settings.show2400 ? 1 : 0)) {
        if (settings.canOver2400) {

        } else if (settings.wrapHours === false) {
          return null;
        } else {
          hours = hour % 24;
        }
